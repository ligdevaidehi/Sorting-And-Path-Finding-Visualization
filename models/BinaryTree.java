...
//        }
    ...
    private void fillTree(TreeNode _parent, int _count, AnchorPane _graph) throws InterruptedException {
        // Base case: Stop recursion when count is 0 or _parent is null
        if (_count <= 0 || _parent == null) {
            return;
        }

        double rand = Math.random() * _count + 1;

        // Update coordinates based on the current node's position
        double offsetX = _count * Math.cos(CHILD_ANGLE);
        double offsetY = _count * Math.sin(CHILD_ANGLE);

        if (rand > _parent.getValue()) {
            // Create right child
            double rightX = _parent.getLayoutX() + offsetX;
            double rightY = _parent.getLayoutY() + offsetY;
            TreeNode node = new TreeNode(rand, rightX, rightY, null, null, _parent);
            _parent.setRightChild(node);
            createAndAddArrow(_parent, node);
            fillTree(node, _count - 1, _graph);
            ...
            // Create left child
            double leftX = _parent.getLayoutX() - offsetX;
            double leftY = _parent.getLayoutY() + offsetY;
            TreeNode node = new TreeNode(rand, leftX, leftY, null, null, _parent);
            _parent.setLeftChild(node);
            createAndAddArrow(_parent, node);
            fillTree(node, _count - 1, _graph);
            _graph.getChildren().add(node);
    ...
    private TreeNode insertVal(TreeNode _root, double _value) {
        ...
        if (_value < _root.getValue()) {
            _root.setLeftChild(insertVal(_root.getLeftChild(), _value));
        } else if (_value > _root.getValue()) {
            _root.setRightChild(insertVal(_root.getRightChild(), _value));
        }

...
public class SortRectangles {
   ...
   /* public static ParallelTransition swapRects(MyRectangle[] _list, int _i, int _j, int _duration) {
        ...
        int move = _j - _i;
        pTran.getChildren().addAll(
                _list[_i].setToX(move * SPACING),
                _list[_j].setToX(-(SPACING * move))
        );
        ...
        // Update the color once they have been visited
        pTran.getChildren().addAll(
                colorRect(_list[_i], Color.BLUE, _duration),
                colorRect(_list[_j], Color.BLUE, _duration)
        );
   ...
   /* private MyRectangle createRect(int _num, Color _color) {
        MyRectangle rect = new MyRectangle();
        rect.setX((SPACING * _num) + SPACING);
        rect.setY(BASE_HEIGHT);

        // Set the height of the first rectangle to the minimum value
        if (_num == 0) {
            rect.setHeight(BASE_HEIGHT);
        } else {
            // Set the heights of the remaining rectangles randomly
            rect.setHeight((Math.random() * (this.grid.getHeight() - 20)) + BASE_HEIGHT);
        }
    ...
    private MyRectangle createRect(int _num, Color _color) {
        ...
        // Set the height to the minimum value for all rectangles initially
        rect.setHeight(BASE_HEIGHT);

...
public class Dijkstras {
    ...
    public List<MyNode> begin(MyNode[][] adj) {
        //this.stran.getChildren().add(Grid.createFill(this.startNode, this.duration, Color.BLUE, Color.));
        ...
        while (this.unvisitedNodes.size() > 0) {
            ...
            colorNode(closestNode, this.startNode, this.finishNode, this.duration, this.stran, Color.BLUE);
    ...
    private void updateUnvisitedNeighbors(MyNode node) {
        ...
        for (MyNode neighbor : neighbors) {
            colorNode(neighbor, this.startNode, this.finishNode, this.duration, this.stran, Color.YELLOW);
